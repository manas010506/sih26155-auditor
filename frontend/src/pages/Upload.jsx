...
const Upload = ({ onAuditComplete }) => {
  ...
  const processFile = (file) => {
    ...
    if (!type) {
      setError(
        `Unsupported file type: "${ext}". Upload a Cisco running-config (.cfg, .txt) or Terraform file (.tf).`
      );
      setStatus('empty');
      setDetected(null);
      return;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      /*
       * Store the audit result in App's shared state.
       * This is what allows the Attack Graph to use
      ...
      /*
       * Go directly to the Attack Graph after the audit.
       */
      setTimeout(() => {
      ...
      setError(
        err.message || 'Failed to process configuration.'
      );
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        >
          Upload a raw configuration file for normalization
          and CIS baseline auditing.
        </div>
      </div>
              ...
              {status === 'empty' && (
                ...
                >
                  ...
                  <div style={{ textAlign: 'center' }}>
                    ...
                    >
                      Cisco IOS (.cfg, .txt) or Terraform
                      (.tf)
                    </div>
