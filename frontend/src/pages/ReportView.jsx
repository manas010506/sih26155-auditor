...
const ReportView = () => {
  ...
  const passedControls = reportData?.passed ?? [];

  // Local date, not UTC: toISOString() gives yesterday's date before 05:30 IST.
  const localDate = () => {
    const d = new Date();
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
  };

  // A config with no hostname would otherwise name the file
  // "compliance_report_Not available in supplied configuration_…".
  const exportName = (ext) => {
    const host = reportData?.device?.hostname;
    const base = !host || host.startsWith('Not available')
      ? (reportData?.source?.filename ?? 'report').replace(/\.[^.]+$/, '')
      : host;
    return `compliance_report_${String(base).replace(/[^\w.-]+/g, '_')}_${localDate()}.${ext}`;
  };
    ...
    const doc = new Document({
      ...
      sections: [{
        ...
        children: [
          ...
          ...(findings.length === 0 ? [
            ...
            return [
              ...
              new Paragraph({
                ...
                children: f.raw_ref?.line
                  ? [
                      new TextRun({ text: `Line ${f.raw_ref.line}: `, bold: true, size: 20, color: '666666' }),
                      new TextRun({ text: f.raw_ref.snippet ?? '', font: 'Consolas', size: 20 }),
                    ]
                  : [new TextRun({ text: 'No direct line reference — global setting or missing configuration.', italics: true, size: 20, color: '666666' })],
              }),
          ...
          ...(assessment(reportData).kind === 'not_assessed' ? [] : [
            new Paragraph({
              text: `Passed Controls (${passedControls.length})`,
              heading: HeadingLevel.HEADING_2,
              spacing: { before: 400, after: 200 },
            }),
            ...(passedControls.length === 0
              ? [new Paragraph({ children: [new TextRun({ text: 'No evaluated control passed.', italics: true })] })]
              : passedControls.map(p => new Paragraph({
                  spacing: { after: 80 },
                  children: [
                    new TextRun({ text: 'PASS  ', bold: true, color: '3FA9A0' }),
                    new TextRun({ text: `${p.rule_id} — `, bold: true }),
                    new TextRun({ text: p.control_ref || p.title || '' }),
                  ],
                }))),
          ]),
          ...
          <div className="glass-card" style={{ padding: '40px 48px', overflow: 'hidden' }}>
            ...
            >
              ...
              <div style={{ marginTop: '20px', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                ...
                {['CIS', 'NIST', 'STIG', 'ISO 27001'].map(fw => {
